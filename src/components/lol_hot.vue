<template>
    <div class="prop_hot">
        <div class="right_title"><div>热门排行</div></div>
        <router-link class="everypro" :to="`/details/${msg.pid}`" v-for="(msg,i) of hot_pro" :key="i">
            <div class="prop">
    ...
    mounted(){
		axios.get(baseURL+"/lol/index_pro").then(result=>{
			...
			for(var item of result.data){
				if (item.prop_place=="hot"){this.hot_pro[n]=item;n++;}
			} 
			this.hot_pro=JSON.parse(JSON.stringify(this.hot_pro))
		})
    }
}
</script>
<style scoped>
.prop_hot{width: 100%;height: 25.9375rem;background: #fafbfc;padding: 0rem 1.125rem 0.5rem;margin-bottom: 0.625rem;}
